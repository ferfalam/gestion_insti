        ...
        <div class="container-fluid">
            <div class="card shadow">
                <div class="card-header py-3">
                    <h3 class="text-primary m-0 fw-bold text-center">Délibérations disponibles</h3>
                </div>
                <div class="card-body">
                    <form action="{{ route('gestion_deliberation.search') }}" method="post">
                       @csrf
                       <div class="row">
                        <div class="col-md-10 text-nowrap d-lg-flex flex-row align-items-lg-center">
                            <div class="dataTables_length" aria-controls="dataTable"><label class="form-label">Filière&nbsp;
                            ...
                            <div class="dataTables_length" aria-controls="dataTable"><label class="form-label">Année&nbsp;<select name="annee" id="annee" class="form-select d-inline-block form-select form-select-sm">
                                @foreach($annees as $annee)
                                <option value="{{$annee->id}}" >{{$annee->name}} </option>
                                @endforeach
                            </select>&nbsp;
                        </label></div>
                        <div class="dataTables_length" aria-controls="dataTable"><label class="form-label">GP&nbsp;<select name="groupe" id="groupe" class="form-select d-inline-block form-select form-select-sm">
                            @foreach($groupes as $groupe)
                            <option value="{{$groupe->id}}" >{{$groupe->name}} </option>
                            @endforeach
                        </select>&nbsp;</label></div>
                        <div class="dataTables_length" aria-controls="dataTable"><label class="form-label">Semestre&nbsp;<select name="semestre" id="semestre" class="form-select d-inline-block form-select form-select-sm">
                        ...
                        <div class="dataTables_length" aria-controls="dataTable"><button class="btn btn-primary" type="submit" onclick="{{$search = true}}" value="rechercher">Rechercher</button></div>
                    </div>
                </div>
            </form>
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <div class="my_jumbotron">
                            <span class="big-s">Aucune délibération trouvée! Essayez une autre recherche.
                            </span>
                        </div>
                        @else
